...
"gRPC 를 쓰면 개발자도구에서 Protobuf 가 어떻게 보이는가."

이 질문에는 함정이 있다. **grpc-gateway 를 쓰는 구성에서는 Protobuf 가 보이지 않는다.** 브라우저는 JSON 을 주고받고 변환은 게이트웨이 안에서 일어난다. 그래서 다섯 경로를 나란히 띄워 무엇이 실제로 다른지 확인했다.
...
2 번 요청은 서버가 gRPC·Protobuf 로 처리한 결과지만 Response 탭에는 JSON 이 보인다. Content-Type 도 `application/json` 이다. **데브툴만 봐서는 백엔드가 gRPC 인지 알 수 없다.**
...
### 2. grpc-gateway 는 Protobuf 의 크기 이득을 반납한다
...
여기서 얻는 결론이 중요하다. **브라우저 to 게이트웨이 구간은 JSON 이므로, Protobuf 의 2.5배 크기 이득이 그 구간에서 사라진다.** grpc-gateway 의 값은 성능이 아니라 호환성이다. 기존 REST 클라이언트를 그대로 두고 서버만 gRPC 로 옮길 수 있게 해준다.

### 3. Protobuf 는 이렇게 보인다
...
**즉 크로스 오리진 구성에서 grpc-web 을 쓰면 Protobuf 크기 이득과 무관한 OPTIONS 왕복 비용이 붙는다.** 완화 수단은 두 가지다.
...
| 브라우저에서 Protobuf 이득까지 얻기 | grpc-web (Envoy `grpc_web` 필터) |
| 서비스 간 내부 통신 | 순수 gRPC. 프록시 불필요 |

브라우저가 끼는 순간 선택지가 둘로 나뉘고, 어느 쪽을 골라도 서비스 간 통신에서 얻던 이득을 그대로 가져오지는 못한다. 이것이 **gRPC 가 프런트엔드 대면 API 보다 서비스 간 통신에서 먼저 채택되는 구조적 이유**다.
